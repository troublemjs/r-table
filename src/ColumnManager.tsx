...
export default class ColumnManager {
  ...
  reset(columns: any[], fixed: boolean | undefined) {
    this._columns = columns.map<ManagedColumn>((column) => {
      ...
      if (column.resizable) {
        ...
        const idx = this._origColumns.findIndex(
          (x: { key: any }) => x.key === column.key,
        );
    ...
    return this._cache('hasFrozenColumns', () => {
      return (
        ...
        this.getVisibleColumns().some(
          (column: { frozen: any }) => !!column.frozen,
        )
      );
    });
  }
  ...
  hasRightFrozenColumns() {
    return this._cache('hasRightFrozenColumns', () => {
      return (
        this._fixed &&
        this.getVisibleColumns().some(
          (column: { frozen: string }) =>
            column.frozen === FrozenDirection.RIGHT,
        )
      );
    });
  }
  ...
  getRightFrozenColumns() {
    return this._cache('rightFrozenColumns', () => {
      if (!this._fixed) return [];
      return this.getVisibleColumns().filter(
        (column: { frozen: string }) => column.frozen === FrozenDirection.RIGHT,
      );
    });
  }

  getColumn(key: null) {
    const idx = this._columns.findIndex(
      (column: { key: any }) => column.key === key,
    );
  ...
  recomputeColumnsWidth(columns: any[]) {
    return columns.reduce(
      (width: any, column: { width: any }) => width + column.width,
      0,
    );
  }
  ...
  recomputeColumnStyles() {
    return this._columns.reduce(
      (styles: { [x: string]: any }, column: { key: string | number }) => {
